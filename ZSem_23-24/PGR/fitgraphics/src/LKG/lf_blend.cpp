...
uint8_t noise(uint8_t x){
  x = x*(x*(x*(x+1)+1)+1);
  return x;
}

void createLF(vars::Vars&vars){
  FUNCTION_PROLOGUE("method",{"method.lf.w","method.lf.h","method.lf.n"});
  auto w = vars.getUint32("method.lf.w");
  auto h = vars.getUint32("method.lf.h");
  auto n = vars.getUint32("method.lf.n");
  std::vector<uint8_t>data(w*h*n*3);
  for(uint32_t z=0;z<n;++z)
    for(uint32_t y=0;y<h;++y)
      for(uint32_t x=0;x<w;++x)
        for(uint32_t c=0;c<3;++c)
          data[(z*w*h+y*w+x)*3+c] = noise(z*3232+y*332+x*17+c);
  auto tex = vars.reCreate<ge::gl::Texture>("method.lf.tex",GL_TEXTURE_3D,GL_RGB8,1,w,h,n);
  tex->setData3D(data.data(),GL_RGB,GL_UNSIGNED_BYTE);
  
}

void onInit(vars::Vars&vars){

  vars.addUint32("method.lf.w",256);
  vars.addUint32("method.lf.h",256);
  vars.addUint32("method.lf.n",8);
  ...
  vars.addString("method.shaderFile",std::string(CMAKE_ROOT_DIR)+"/resources/shaders/lf_blend.glsl");
...
void onDraw(vars::Vars&vars){
  ...
  auto lf_tex = vars.get<ge::gl::Texture>("method.lf.tex");
  lf_tex->bind(0);
...
EntryPoint main = [](){
  ...
  MethodRegister::get().manager.registerMethod("lkg.lf_blend",clbs);
};
