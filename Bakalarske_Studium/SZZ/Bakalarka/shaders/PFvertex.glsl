...
void hemisphere(uint vID,uint shID,in mat4 vp,in mat4 model,in uint nCols,in uint nRows,in vec3 center,in float radius){
  ...
  if (SHColor == 0){
    v_color   = normalize(vec3(inverse(transpose(model))*vec4(sphere,0)));
  }
  else if (SHColor == 1){
    v_color   = normalize(vec3(0.0 + shR, 1.0 - shR, 0.0));
  }
...
void main()
{
    ...
    if (switcher == 0){
      ...
      hemisphere(vID,shID,vp,model,ncols,nrows,vec3(0,0,0),shScale * size);
  }
}